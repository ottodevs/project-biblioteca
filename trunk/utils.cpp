...
void Utils::cargarUsuarios(QWidget *a){

    j=0;
    k=0;

    nombreArchivo = QFileDialog::getOpenFileName(a,("Abrir archivo"),QDir::homePath(),("csv(*.csv)"));
    QFile archivo(nombreArchivo);
    ...
    while(!archivo.atEnd()){
        ...
        QSqlQuery sql;
        sql.exec("INSERT INTO personas values('"+this->cota+"',\""+this->autor+"\",\""+this->titulo
                 +"\",\""+this->materia+"\"); COMMIT");
    }
...
void Utils::guardarUsuarios(QWidget *a){

    nombreArchivo = QFileDialog::getSaveFileName(a,("Guardar Archivo"),QDir::homePath(),("csv(*.csv)"));
    QFile archivo(nombreArchivo);

    archivo.open(QFile::WriteOnly | QFile::Text);

    QSqlQuery sql;
    sql.exec("select * from personas");
    ...
    while(sql.next()){
        ...
        archivo.write(sql.value(0).toByteArray());
        archivo.write(";");
        archivo.write(sql.value(1).toByteArray());
        archivo.write(";");
        archivo.write(sql.value(2).toByteArray());
        archivo.write(";");
        archivo.write(sql.value(3).toByteArray());
        archivo.write("\n");
    }
